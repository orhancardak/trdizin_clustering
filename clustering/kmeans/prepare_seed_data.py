...
id_to_row = articles.set_index("external_id")["embedding_row"].to_dict()
...
def select_representative_articles(candidate_ids, count):
    ...
    selected_ids = [valid_ids[i] for i in order[:selected_count]]
...
for subject_name in leaf_subjects:
    subject_article_ids = (
        subjects[subjects["subject_fullname"] == subject_name]["external_id"]
        .drop_duplicates()
        .tolist()
    )
    ...
    if len(clean_candidates) >= SEEDS_PER_SUBJECT:
        selected_ids = select_representative_articles(
            clean_candidates,
            SEEDS_PER_SUBJECT
        )
        selection_type = "REPRESENTATIVE_SINGLE_LABEL"
    ...
    # ========================================================
        ...
        extra = select_representative_articles(
            remaining_candidates,
            needed
        )
        ...
        if len(clean_candidates) > 0:
            selection_type = "REPRESENTATIVE_MIXED"
        else:
            selection_type = "REPRESENTATIVE_MULTI_LABEL_ONLY"
    ...
    for article_id in selected_ids:
        ...
        embedding_row = int(id_to_row[article_id])
        selected_embedding_rows.append(embedding_row)

        seed_rows.append({
            "subject_fullname": subject_name,
            "external_id": article_id,
            "embedding_row": embedding_row,
            "article_label_count": int(label_counts.get(article_id, 0)),
    ...
    if not selected_embedding_rows:
        print("UYARI - seed bulunamadı:", subject_name)
        continue

    subject_vectors = embeddings[selected_embedding_rows]
    centroid = subject_vectors.mean(axis=0)
...
centroid_metadata = pd.DataFrame({
    "cluster_id": range(len(centroid_subjects)),
    "subject_fullname": centroid_subjects
})
...
print("Centroid oluşturulan konu:", len(centroid_subjects))
print("Centroid matrix shape:", centroid_matrix.shape)
print("Toplam seed ilişkisi:", len(seed_df))
print("Benzersiz seed makale:", seed_df["external_id"].nunique())

print("\nSEÇİM TİPLERİ")
print("-" * 70)
print(seed_df["selection_type"].value_counts().to_string())

# ============================================================
# SEED SAYISI
# ============================================================

seed_counts = seed_df.groupby("subject_fullname")["external_id"].nunique()

print("\nSEED SAYISI")
print("-" * 70)
print(seed_counts.describe())

print("\n5'TEN AZ SEED OLAN KONULAR")
print("-" * 100)
low_seed = seed_counts[seed_counts < SEEDS_PER_SUBJECT]

if low_seed.empty:
    print("Yok")
else:
    print(low_seed.to_string())
...
seed_df.to_csv(
    os.path.join(OUTPUT_DIR, "seed_articles.csv"),
    index=False,
    encoding="utf-8-sig"
)

centroid_metadata.to_csv(
    os.path.join(OUTPUT_DIR, "seed_centroid_subjects.csv"),
    index=False,
    encoding="utf-8-sig"
)

np.save(
    os.path.join(OUTPUT_DIR, "seed_centroids.npy"),
    centroid_matrix
)
...
print("TAMAMLANDI")
print("=" * 100)
print("results/kmeans/seed_articles.csv")
print("results/kmeans/seed_centroid_subjects.csv")
print("results/kmeans/seed_centroids.npy")
...
PREDICTION_PATH = BASE_DIR / "results" / "kmeans" / "holdout" / "adaptive_v2_relabel_predictions.csv"
OUTPUT_DIR = BASE_DIR / "results" / "kmeans"
COORDINATE_OUTPUT = OUTPUT_DIR / "kmeans_v2_umap_coordinates.csv"
HTML_OUTPUT = OUTPUT_DIR / "kmeans_v2_umap_interactive.html"
...
index_df = pd.read_csv(INDEX_PATH, dtype={"external_id": str})
prediction_df = pd.read_csv(PREDICTION_PATH, dtype={"external_id": str})

index_df["external_id"] = index_df["external_id"].astype(str).str.strip()
prediction_df["external_id"] = prediction_df["external_id"].astype(str).str.strip()

print("Tüm embedding shape:", embeddings.shape)
print("Final V2 tahmin makalesi:", len(prediction_df))
...
plot_df = prediction_df.merge(
    index_df[["embedding_row", "external_id", "doi", "language", "title"]],
    on="external_id",
    how="inner"
)

plot_df = plot_df.drop_duplicates(subset=["external_id"]).reset_index(drop=True)
embedding_rows = plot_df["embedding_row"].astype(int).to_numpy()
X = embeddings[embedding_rows]

print("UMAP'a girecek makale:", len(plot_df))
print("UMAP input shape:", X.shape)

if len(plot_df) != len(prediction_df):
    print("UYARI:", len(prediction_df) - len(plot_df), "tahmin makalesi embedding index ile eşleşmedi.")
...
print("UMAP output shape:",coordinates.shape)

# ------------------------------------------------------------
# 4. GRAFİKTE KULLANILACAK ALANLAR
# ------------------------------------------------------------

plot_df["Ana_Konu"] = plot_df["main_level_2_predictions"].apply(first_topic)
plot_df["Alt_Konular"] = plot_df["leaf_predictions"].apply(clean_text)
plot_df["Gercek_Konular"] = plot_df["true_subjects"].apply(clean_text)
plot_df["title"] = plot_df["title"].fillna("")
plot_df["doi"] = plot_df["doi"].fillna("")
plot_df["language"] = plot_df["language"].fillna("")
...
plot_df[[
    "external_id", "embedding_row", "UMAP_1", "UMAP_2",
    "Ana_Konu", "main_level_2_predictions", "leaf_predictions",
    "true_subjects", "title", "doi", "language"
]].to_csv(COORDINATE_OUTPUT, index=False, encoding="utf-8-sig")

print()
print("Koordinat CSV:", COORDINATE_OUTPUT)
...
print("Interaktif HTML:", HTML_OUTPUT)
print()
print("=" * 110)
print()
print("=" * 110)
print("TAMAMLANDI")
print("=" * 110)
print("Grafikteki nokta sayısı:", len(plot_df))
print("Benzersiz ana konu:", plot_df["Ana_Konu"].nunique())